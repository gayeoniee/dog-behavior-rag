...
렌더러를 붙이지 않은 이유는 답변 형식이 고정돼 있어(진단/이렇게 해보세요/주의점)
...
# 답변 폼의 **마지막** 구획. 문제행동은 "주의점:", 훈련법은 "포인트:",
# 되묻기는 "가능한 원인:"으로 끝난다.
# "피하세요"는 옛 라벨이다. 프롬프트를 바꿔도 모델이 습관적으로 옛 라벨을 뱉는 일이
# 있고, 그때 이 정규식이 못 맞추면 **사족 잘라내기가 조용히 죽는다.** 남겨둔다.
_FORM_TAIL = re.compile(r"^[ \t]*(주의점|피하세요|포인트|가능한 원인)\s*[:：]", re.MULTILINE)
