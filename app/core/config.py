...
class Settings(BaseSettings):
    ...
    **비워두면 추론형 로컬 모델에서 짧은 호출이 전부 빈 응답이 된다.** gemma-4-e2b로
    ...
    끄면 코퍼스가 영어뿐이던 시절 그대로 영어 한 줄만 만든다 — A/B의 대조군이다.

    측정 근거: 한 벡터에 섞으면 어느 쪽도 제대로 못 친다. 한국어 풀 안에서만 봐도
    구어체 질문은 식분증 문서를 16위에 두는데, 문서체로 다시 쓰면 1위(0.734)다.
    질문은 구어체고 문서는 문어체라 서로 안 만난다.
    """
