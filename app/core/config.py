...
class Settings(BaseSettings):
    ...
    **비워두면 추론형 로컬 모델에서 짧은 호출이 전부 빈 응답이 된다.** gemma-4-e2b로
    ...
    bilingual_query_enabled: bool = True
    """재작성이 **언어마다 하나씩** 질의를 만들지 (영어 논문용 · 한국어 자막용).

    측정 (2026-08-18, 581문항 · 같은 날 두 판을 겨뤘다):

        1차 고정 틀     hit@5 42.5%   ← 기각. 대조군보다 -5.2%p
        대조군          hit@5 47.7%
        2차 최소 수정   hit@5 51.8%   ← 채택. **+4.1%p** (잡음 기준 0.7%p)

    **차이는 KO 프롬프트 하나였다.** 1차는 모든 질문을 `반려견이 X하는 행동의
    원인과 교정 방법` 한 틀에 밀어넣어 "문제행동 → 교정" 프레임을 강요했다.
    2차는 문체만 바꾸고 내용어를 전부 보존한다 (`REWRITE_SYSTEM_KO` 참조).

    32문항은 25·24 (대조군 24)로 사실상 동률인데, **오래 실패하던 항목들이
    새로 통과했다** — 식분증·입마개·주저앉기·억제된 배변.

    ⚠️ **out-of-scope 7문항 중 2개는 이 값으로 판정하면 안 된다.** 범위 판정
    자체가 흔들린다 (훈련사 자격증 DOG 1/6, 슬개골 3/6 — 같은 입력, 6회 반복).
    결정적인 5문항은 양쪽 다 5/5다. 범위 판정은 원문 질문만 받으므로 이 설정과
    무관하다는 것이 코드 경로로도 증명된다 (`evidence_select._ask_domain`).
    """
