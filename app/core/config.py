...
class Settings(BaseSettings):
    ...
    **비워두면 추론형 로컬 모델에서 짧은 호출이 전부 빈 응답이 된다.** gemma-4-e2b로
    ...
    bilingual_query_enabled: bool = False
    """재작성이 **언어마다 하나씩** 질의를 만들지 (영어 논문용 · 한국어 자막용).

    **기본값이 꺼짐인 이유 (2026-08-18 A/B).** 손으로 쓴 32문항은 좋아졌는데
    자동 581문항이 크게 나빠졌다:

        32문항 (보호자 말투)   24 → 25   식분증·억제된 배변이 새로 통과
        581문항 (청크 생성)    hit@5 48.7% → 42.5%   (-6.2%p, 잡음 기준 0.7%p)

    **원인: 문서 균질화를 고치려다 질의를 균질화했다.** KO 재작성이 모든 질문을
    `반려견이 X하는 행동의 원인과 교정 방법` 한 틀에 밀어넣는다. 구어체로 뭉뚱하게
    묻는 질문에는 문체를 맞춰줘서 도움이 되지만(똥 16위 → 1위), 이미 구체적인
    질문에서는 **구별되는 말을 깎아** 검색을 망친다.

    켤 가치는 있다 — 방향은 맞았다. 다시 하려면 **고정 틀을 버리고** 문체만
    맞추는 KO 프롬프트가 필요하다. 배선·대조군·측정 도구는 다 남겨뒀다.
    """
