...
- ✅ **검색 품질 평가셋** (`scripts.eval.retrieval_report`, 20문항). 기준선 10/20 →
  **로컬 19/20 · Gemini 18/20** (둘 다 out-of-scope 4/4). 남은 실패는 뛰어오르기 —
  네 번의 실행에서 네 번 다 같은 문서로 실패한 **유일하게 안정적인 공백**이다.
  전용 문서가 없고, 재주 훈련과 함께 라이선스 깨끗한 소스가 없다
  (판단 근거는 `sources.yaml`)
...
- **흔들리는 항목은 리콜 하나다.** RSPCA 리콜 문서가 네 번 중 두 번 1위로 올라오고,
  나머지 두 번은 ASPCA 공격성 문서에 밀린다. 평가셋의 옛 주석("논문에 밀려 상위에
  못 온다")은 `doc_type` 부스트 이전 것이라 지금은 절반만 맞다 — 주석도 데이터처럼
  낡는다. 다음 과제는 이 랭킹 흔들림 자체다
