...
- ✅ **LLM 연결 완료.** `openai-compatible` provider. **현재 `.env`는 LM Studio
  로컬(`google/gemma-4-e2b`)을 가리킨다** — 18/20에 20초. Gemini 무료 티어
  (`gemini-3.1-flash-lite`)는 19/20에 5초이고 주석으로 남겨뒀다 (아래 "로컬 LLM" 참조)
...
- ✅ **검색 품질 평가셋** (`scripts.eval.retrieval_report`, 20문항). 기준선 10/20 →
  **Gemini 19/20 · 로컬 gemma-4-e2b 18/20** (둘 다 covered 14/14, out-of-scope 4/4).
  Gemini 기준 남은 1건은 뛰어오르기 —
...
LM Studio + `google/gemma-4-e2b`(4.6B)로 바꿔보고 측정했다. 결론은 **19/20 → 18/20,
5초 → 20초.** 되돌리려면 `.env`의 Gemini 주석 세 줄을 살리고 `EMBEDDING_DEVICE=auto`.
