...
- **점수로는 안 보이는 격차가 멀티턴에 있다.** 평가셋은 사실상 동률인데 같은 4턴에서:

      반복 경고        gemma 1건   Gemini 0건
      3번 겹침         gemma 83%   Gemini 17%
      주의점(4턴)      gemma 3문장  Gemini 4문장

  조언의 구체성도 다르다 — Gemini "켄넬 근처를 지날 때마다 간식을 안으로 던져주세요"
  vs gemma "반려견의 반응을 관찰하세요". **프롬프트를 강화해도 3번 83%는 안 내려갔다**
  (4번 100% → 17%는 잡혔다). 같은 프롬프트로 Gemini가 해내므로 모델 한계이고,
  더 조이면 다른 게 깎이므로 멈췄다
