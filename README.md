...
| LLM | OpenAI 호환 프로토콜 — Gemini 무료 티어 또는 LM Studio 로컬(`gemma-4-e2b`). `LLM_BASE_URL`만 바꾸면 교체 |
...
- **자료가 없는 주제를 아직 못 걸러낸다.** "혼자 있을 때 배변을 안 해"(억제된 배변)는
  코퍼스에 반대 패턴("혼자 있을 때 실내에 배변한다")만 있다. 그런데 근거 선별이
  "혼자·배변"이라는 겉모양만 보고 그 문서를 근거로 붙여 답한다 — 로컬 gemma로 4번
  물어 4번 다 그랬고, 한 번은 질문의 뜻을 거꾸로 읽었다(2026-09-15).
  프롬프트 규칙·판정 분리·영어 재진술 세 가지로 고쳐봤지만 셋 다 안 됐고, **같은
  프롬프트를 Gemini로 돌리니 3/3 거절했다** — 설계가 아니라 4.6B 모델의 한계였다.
  근거 선별만 더 큰 모델로 보내는 혼합 구성이 다음 후보다
