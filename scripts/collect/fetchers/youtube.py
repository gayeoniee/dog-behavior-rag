...
정리하면 **막힌 것은 엔드포인트가 아니라 "로그인하지 않은 우리 IP"다.**

**해법은 휴대폰 핫스팟이었다 (2026-08-16).** 집·학원 두 IP가 다 막힌 상태에서
노트북을 폰 핫스팟에 물리니 바로 받아졌다. 통신사 IP는 쿼터가 따로다.

이게 이 사건의 교훈이다. **문제를 코드에서 일곱 번 찾았는데 코드에 없었다.**
백오프·클라이언트 교체·JS 런타임·EJS·오디오 우회를 차례로 시도하는 동안,
"IP 단위 차단"이라고 스스로 진단해 놓고도 **IP를 바꿔볼 생각을 안 했다.**
진단이 맞았는데 그 진단이 가리키는 행동을 안 한 것이다.

막힌 게 우리 코드가 아닐 때는 **바꿀 수 있는 것이 코드 말고 또 뭐가 있는지**
부터 세어볼 것. 지연을 더 늘리는 여덟 번째 시도는 하지 말 것.
"""
