# dog-behavior-rag 사용 설명서

프로젝트 소개는 [README](../README.md)에 있다. 이 문서는 설치·수집·적재·평가·LLM
설정을 다루는 매뉴얼이다.

> 📚 **이 프로젝트가 처음이라면 [`learn/00-checklist.md`](learn/00-checklist.md)부터.**
> RAG가 무엇인지, 이 레포의 데이터가 어디서 와서 어디에 있는지를 실제 숫자로 따라가는
> 학습 문서다.

> ⚠️ **날짜가 붙은 절은 그 시점의 기록이다.** 코퍼스는 이후 622건 / 청크 9,911개로
> 늘었고(한국어 영상 자막 328편 추가), 평가셋도 32문항 + 자동 생성 581문항으로 커졌다.
> 아래 "유튜브·블로그는 수집하지 않는다"는 원칙은 2026-08-15에 바뀌었다 — 경위는
> `data/sources.yaml`의 보듬TV 항목 주석에 있다.
...
**그리고 이 표가 멀티턴을 안 잰다.** 같은 4턴 대화를 재현하면(`scripts.eval.replay`)
단발 점수에 안 잡히는 것이 보인다. 처음 쟀을 때는 이랬다:

| | gemma | Gemini |
|---|---|---|
| 반복 경고 (이전 답변과 50% 이상 겹침) | 1건 | 0건 |
| 3번 답변이 1번과 겹치는 비율 | 83% | 17% |

이걸 모델 격차로 읽었는데 **틀렸다.** 답변 폼이 판단형 질문("그러면 분리불안인건가?")을
담지 못해 억지로 단계를 채우다 앞 답변을 복사한 것이었고, 폼의 첫 줄을 일반화하자
gemma도 3번 33% / 반복 경고 0건이 됐다. Gemini는 같은 결함을 능력으로 가리고 있었을
뿐이다. **강한 모델이 통과한다고 설계가 멀쩡한 것은 아니다.**

남은 격차는 조언의 구체성이다 — Gemini "켄넬 근처를 지날 때마다 간식을 안으로
던져주세요" vs gemma "반려견의 반응을 관찰하세요".

**±1점은 잡음이다.** 질의 재작성이 LLM이라 실행마다 문구가 달라지고, 그게 검색
순위를 바꾼다. 다섯 번의 실행을 항목별로 겹쳐 보면:

- **리콜** — RSPCA "Train Your Dog To Come When Called" 문서가 다섯 번 중 두 번
  1위로 올라오고, 나머지는 ASPCA 공격성 문서에 밀린다. 재작성 문구에 따라 갈리는
  **랭킹 흔들림**이고, 유일하게 실행마다 뒤집히는 항목이다
...
- **뛰어오르기** — 다섯 번 다 같은 문서, 다섯 번 다 실패. **이것만 진짜 코퍼스 공백이다**

총점 한 자리를 비교하기 전에 어느 항목이 움직였는지 보라는 뜻이다.

**키워드가 판정을 헐겁게 만들지 않는지 같이 본다.** `covered` 판정은 남은 근거에
키워드가 하나라도 있으면 통과라, 흔한 단어(`fear` 16.7%)나 부분 문자열
(`tail`←detail)이 섞이면 근거가 부실해도 통과한다. 2026-08-13에 전부 정비해
모든 키워드를 7% 미만으로 맞췄고, `judge()`는 **단어 시작 경계**로 본다
(`retrieval_report.mentions` — 앞쪽만 걸어야 `bark`가 `barking`을 계속 잡는다).
새 질문을 넣을 때 이 두 가지를 확인할 것.
...
# 멀티턴 재현 — 위 리포트가 못 보는 층 (서버가 떠 있어야 한다)
uv run python -m scripts.eval.replay v2 --name night-barking-crate
```
...
**단발 질문의 검색·선별까지만 잰다.** 답변 문장도, 대화가 이어질 때의 동작도 보지
않는다. 실제로 그 사각지대에서 세 가지가 무너져 있었고(프롬프트 지시문이 답변에
그대로 출력, 앞 답변 재사용, 후속 질문이 되묻기로 흘러감) 전부 사용자가 직접
대화해 보고서야 발견됐다.

그래서 `scripts.eval.replay`가 짝으로 있다. `data/replay_conversations.yaml`의
대화를 순서대로 재생하며 기계적으로 확인 가능한 것만 검사한다 — 지시문 누출,
이전 답변과의 겹침 비율, 한 라벨이 매 턴 같은 말인지.

**완전 일치만 보면 놓친다.** 처음엔 그렇게 만들었다가, 진단·주의점을 재사용하고
단계 하나만 바꾼 답변이 검사를 통과했다. 사람 눈에는 같은 답인데 통과한 것이라
문장 단위 겹침 비율로 바꿨다.
